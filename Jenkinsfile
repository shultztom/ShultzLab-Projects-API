    ...
    withCredentials([usernamePassword(credentialsId: 'nginx-username-password', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
        ...
        stage('Build on Server and Deploy'){
            if(env.BRANCH_NAME == 'master'){     
                ...
                sshCommand remote: remote, command: "cd /home/tks23/tmp/${workspaceFolder} && . ~/.nvm/nvm.sh && nvm install && nvm use && npm i yarn -g && yarn install"
                ...
                sshCommand remote: remote, command: "cd /home/tks23/projects/ShultzLab-Projects-API/ && node -v && pm2 reload shultzlab-api"
                sshCommand remote: remote, command: "rm -rf /home/tks23/tmp/${workspaceFolder}"
            }else{
